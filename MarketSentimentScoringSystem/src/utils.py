...
def dump_file(file, target_date, aws_env=None):
    # dump file
    if target_date=='now':
        target_date = datetime.now(timezone('Asia/Seoul'))
    else:
        target_date = target_date

    if aws_env:
        dump_file_path = f"tmp/{target_date.strftime('%Y-%m-%d')}.json"
    else:
        dump_file_path = f"impact score/{target_date.strftime('%Y-%m-%d')}.json" ## summarizer 
        # dump_file_path = f"/impact score/{now_time.strftime('%Y-%m-%d')}.json" ## impact score
