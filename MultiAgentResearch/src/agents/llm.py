...
from src.utils.token_tracker import token_tracker  # 토큰 추적기 import
...
class llm_call():
    ...
    def invoke(self, **kwargs):
        ...
        # 토큰 사용량 추적
        try:
            if hasattr(response, 'get') and response.get('token_usage'):
                token_usage = response['token_usage']
                
                # 캐시 사용 여부 확인 (에이전트별 설정 기반)
                cache_used = False
                if agent_name and agent_name in token_tracker.agent_cache_map:
                    cache_used = token_tracker.agent_cache_map[agent_name]
                
                # 도구 호출 횟수 계산
                tool_calls = 0
                if response.get('toolUse'):
                    tool_calls = 1
                
                # Reasoning 토큰 계산
                reasoning_tokens = 0
                if response.get('reasoning'):
                    reasoning_tokens = len(response.get('reasoning', '')) // 4  # 대략적인 토큰 수 추정
            ...
            # 토큰 추적 실패 시 오류를 로그하되 메인 기능에는 영향 없음
            print(f"⚠️ 토큰 추적 실패: {tracking_error}")
...
def get_llm_by_type(llm_type: LLMType):
    ...
    if llm_type == "reasoning":
        llm = bedrock_model(
            # model_id=bedrock_info.get_model_id(model_name="Claude-V4-1-Opus-CRI"),
            model_id=bedrock_info.get_model_id(model_name="Claude-V4-1-Sonnet-CRI"),
