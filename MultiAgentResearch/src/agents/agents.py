...
if llm_module == "src.agents.llm_st":
    from src.agents.llm_st import get_llm_by_type, llm_call
else:
...
class create_react_agent:
    ...
    def __init__(self, **kwargs):
        ...
        if self.agent_name == "researcher":
            self.tool_config = research_tool_config
        elif self.agent_name == "coder":
            self.tool_config = coder_tool_config
        ...
        # 반복 대화 처리를 위한 설정
        self.MAX_TURNS = 80  # 무한 루프 방지용 최대 턴 수
        # Reporter의 경우 더 적은 턴 수로 제한
        if self.agent_name == "reporter":
            self.MAX_TURNS = (
                25  # Reporter는 최대 20턴으로 늘림 (점진적 보고서 작성 고려)
            )
        self.turn = 0
    ...
    def invoke(self, **kwargs):
        ...
        system_prompts, messages = apply_prompt_template(
            self.agent_name, state, prompt_cache=prompt_cache, cache_type=cache_type
        )
        ...
        while not self.final_response and self.turn < self.MAX_TURNS:
            ...
            if response["stop_reason"] == "tool_use":
                ...
                for content in ai_message["content"]:
                    if "toolUse" in content:
                        ...
                        if self.agent_name == "researcher":
                            ...
                            tool_result_message = process_coder_tool(tool)
                        # elif self.agent_name == "browser": tool_result_message = process_browser_tool(tool)
                        elif self.agent_name == "reporter":
                            tool_result_message = process_reporter_tool(tool)
                            # Reporter가 최종 보고서 생성 도구를 사용했으면 종료
                            if tool["name"] in [
                                "generate_html_report",
                                "generate_markdown_report",
                                "finalize_progressive_report",
                            ]:
                                logger.info(
                                    f"{Colors.GREEN}Reporter - 최종 보고서 생성 완료, 대화 종료{Colors.END}"
                                )
                                self.final_response = True

                        messages.append(tool_result_message)
                        logger.info(
                            f"{Colors.BOLD}ToolUse - 도구 실행 결과를 대화에 추가했습니다.{Colors.END}"
                        )
                ...
                if not tool_requests_found:
                    ...
                    logger.info(
                        f"{Colors.UNDERLINE}ToolUse - 도구 요청을 찾을 수 없습니다.{Colors.END}"
                    )
                    self.final_response = True
            else:
                ...
                self.final_response = True
                logger.info(
                    f"{Colors.UNDERLINE}ToolUse - 최종 응답을 받았습니다.{Colors.END}"
                )
                print("최종 응답을 받았습니다.")
        ...
        if self.turn >= self.MAX_TURNS:
            print(
                f"⚠️  경고: 최대 턴 수({self.MAX_TURNS})에 도달하여 대화가 강제 종료되었습니다."
            )
            logger.warning(
                f"{Colors.RED}Agent {self.agent_name} - 최대 턴 수 도달로 강제 종료{Colors.END}"
            )
        print("최종 응답:\n", response)
        print("메시지:\n", ai_message)
