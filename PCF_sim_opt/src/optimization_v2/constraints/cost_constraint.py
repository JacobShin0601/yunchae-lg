...
class CostConstraint(ConstraintBase):
    ...
    def __init__(self, cost_calculator=None):
        ...
        Args:
            cost_calculator: TotalCostCalculator 인스턴스 (optional, 직렬화 후 로드 시 None 가능)
        """
        super().__init__(
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        Note:
            cost_calculator는 직렬화하지 않음 (복잡한 객체이므로)
            로드 후 set_cost_calculator()로 재설정 필요

        Returns:
            설정 딕셔너리
        """
        return {
            'type': 'cost_constraint',  # Registry와 일치하도록 수정
            'name': self.name,
